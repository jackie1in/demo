...
public class RetryQueuesInterceptor implements MethodInterceptor {
    private static Logger logger = LoggerFactory.getLogger(RetryQueuesInterceptor.class);
    ...
    private Object tryConsume(MethodInvocation invocation, Consumer<MessageAndChannel> successHandler, BiConsumer<MessageAndChannel, Throwable> errorHandler){
    ...
    private int tryGetRetryCountOrFail(MessageAndChannel mac, Throwable originalError) throws Throwable {
        ...
        if (retryQueues.retriesExhausted(xRetriedCount)) {
            ...
            throw new RetriesExhaustedException(originalError);
        }
